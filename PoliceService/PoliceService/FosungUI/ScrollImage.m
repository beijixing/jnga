...
- (instancetype)initWithCurrentController:(UIViewController *)viewcontroller urlString:(NSArray *)urls viewFrame:(CGRect)frame placeholderImage:(UIImage *)image title:(NSArray *)titles {
    if (self = [super init]) {
        [viewcontroller addChildViewController:self];
        _timeInterval     = 1.0;
        _imageCount       = urls.count;
...
- (UIView *)maskView {
    ...
    _maskView = [[UIView alloc] initWithFrame:CGRectMake(0,  _frame.size.height-40, _frame.size.width, 40)];
...
- (UILabel *)titleLabel {
    ...
    _titleLabel = [[UILabel alloc] initWithFrame:CGRectMake(15, _frame.size.height-40, _frame.size.width/4*3-15, 40)];
...
{
    if (_pageControl == nil) {
        _pageControl = [[UIPageControl alloc] initWithFrame:CGRectMake(_frame.size.width/4*3, _frame.size.height-30, _frame.size.width/4, 20)];
...
- (void)tapImage:(UITapGestureRecognizer *)gesture
{
    if ([self.delegate respondsToSelector:@selector(scrollImage:clickedAtIndex:)]) {
        [self.delegate scrollImage:self clickedAtIndex:_pageControl.currentPage];
    }
}
...
- (void)scrollViewDidEndDecelerating:(UIScrollView *)scrollView
{
    ...
    if (_pageControl.currentPage<_titleArr.count) {
        self.titleLabel.text = _titleArr[_pageControl.currentPage];
    }
}
