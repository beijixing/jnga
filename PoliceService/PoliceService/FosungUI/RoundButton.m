...
- (instancetype)initWithFrame:(CGRect)frame {
    if (self = [super initWithFrame:frame]) {
        ...
        self.kScale = [UIScreen mainScreen].bounds.size.width/640;
...
- (void)buttonClick:(UIButton *)sender {
    if (self.actionBlock) {
        self.actionBlock(self.tag, self.titleLabel.text);
    }
}
...
- (void)layoutSubviews {
    ...
    [self.cirCleButton mas_makeConstraints:^(MASConstraintMaker *make) {
        make.size.mas_equalTo(CGSizeMake(self.frame.size.height - 40*self.kScale, self.frame.size.height - 40*self.kScale));
